...
run_iterative_mle <- function(
    ...
    # --- 1. 数据预处理 ---
    # 将输入矩阵转置为 2xM 格式，便于按列（SNP）处理
    gamma_hat <- t(beta_hat_exp)
    beta_hat <- t(beta_hat_out)

    # 将堆叠的协方差逆矩阵解析为列表，每个元素是一个 2x2 矩阵
    # 同时计算并存储原始的协方差矩阵，因为后续步骤需要
    ...
    for (iter in 1:max_iter) {
        ...
        # 提取计算异质性统计量所需的方差 (Sigma_beta 的对角线元素)
    ...
    return(list(
        ...
        gamma_final = t(gamma_current), # 转置回 M x 2 格式
        iterations = iter,
        converged = converged,
        sets = final_sets,
