...
Triplet_Family_Simulation_Once <- function(
    n = 10, num_pleiotropic = 5, N_out = 4000,
    # 直接效应
    beta_FStoOE_exp = 0.3, beta_MStoOE_exp = 0.3, beta_OStoOE_exp = 0.3,
    # 水平多效性
    prop_negative_pleiotropy = 0.5, # 不满足inside假设
    ...
    ## 选型婚配暴露相关
    compatibility_selection_factor_exp = 0, compatibility_selection_factor_out = 0,
    # 人群分层（双人群差异）
    crowd_stratification_differences = 0,
    # 其他参数设置
    beta_exp_to_out = 0, beta_confounding_exp = 0.2, beta_confounding_out = 0.2, correlation = 0.2, seed = NULL) {
...
results <- list(
    ...
    J_theta_point = NA, J_theta_se = NA, J_z = NA, J_p_value = NA, J_duration = NA,
    K_theta_point = NA, K_theta_se = NA, K_z = NA, K_p_value = NA, K_duration = NA
) # 初始化结果列表
...
Phase_One_Data_oracle <- Phase_One_Data %>% filter(snp_type != "pleiotropic_variable")
Phase_Two_Data_oracle <- Phase_Two_Data %>% filter(snp_type != "pleiotropic_variable")
# 进行FGWAS处理
...
res_C <- mr_presso(BetaOutcome = "beta_out", BetaExposure = "beta_exp", SdOutcome = "se_out", SdExposure = "se_exp", OUTLIERtest = TRUE, data = presso_method_input, DISTORTIONtest = TRUE, NbDistribution = 1000, SignifThreshold = 0.05)
...
results$E_Euration <- as.numeric(res_E_time_2 - res_E_time_1)
...
results$F_Furation <- as.numeric(res_F_time_2 - res_F_time_1)
...
results$G_Guration <- as.numeric(res_G_time_2 - res_G_time_1)
...
results$I_p_value <- res_I$Causal.pval
results$I_Iuration <- as.numeric(res_I_time_2 - res_I_time_1)
